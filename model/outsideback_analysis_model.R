...
# Jogadores zagueiros laterais (Outside-back) que atuam no continente europeu
OBACK_EUROPE <- df %>%
  filter(Position == "Outside-back" & continent == "Europe")

# Dataframe para validação - Jogadores zagueiros laterais que NÃO jogam na europa
OBACK_NOT_EUROPE <- df %>%
  filter(Position == "Outside-back" & continent != "Europe")
...
# Boxplot dos valores de 'OBACK_EUROPE' ... Outliers no valor em euros
boxplot(OBACK_EUROPE)

# Matrix de corelação das habilidades e características dos jogadores selecionados em OBACK_EUROPE
...
reg <- tree(data = OBACK_EUROPE, formula = eur_value ~ .)
plot(reg)
text(reg)
summary(reg)
